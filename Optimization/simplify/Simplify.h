...
class SymetricMatrix { 
	...
	SymetricMatrix(double c=0) { loopi(0,10,1) m[i] = c;  }
...
//		int timeStart=timeGetTime();
		...
		loopi(0,(signed int)triangles.size(),1) triangles[i].deleted=0;
		...
		loop(iteration,0,100) 
		{
			...
			// clear dirty flag
			loopi(0,(signed int)triangles.size(),1) triangles[i].dirty=0;
			...
			loopi(0,(signed int)triangles.size(),1)
			{				
				...
				loopj(0,3,1)if(t.err[j]<threshold) 
				{
...
//		int timeEnd=timeGetTime();
	...
	{		
		...
		if( iteration == 0 )
		{
			loopi(0,(signed int)vertices.size(),1) 
			...
			loopi(0,(signed int)triangles.size(),1) 
			{
				Triangle &t=triangles[i]; 
				vec3f n,p[3];
				loopj(0,3,1) p[j]=vertices[t.v[j]].p;
				n.cross(p[1]-p[0],p[2]-p[0]);
				n.normalize();
				t.n=n;
				loopj(0,3,1) vertices[t.v[j]].q = 
			...
			loopi(0,(signed int)triangles.size(),1)
			{
				...
				loopj(0,3,1) t.err[j]=calculate_error(t.v[j],t.v[(j+1)%3],p);
				t.err[3]=min(t.err[0],min(t.err[1],t.err[2]));
			}	
		}
		...
		{
			Triangle &t=triangles[i];
			loopj(0,3,1) vertices[t.v[j]].tcount++;
		}
		int tstart=0;
		loopi(0,(signed int)vertices.size(),1)
		{
		...
		if( iteration == 0 )
		{
			...
			loopi(0,(signed int )vertices.size(),1)
			{
				...
				loopj(0,v.tcount,1)
				{
					...
					{
						int ofs=0,id=t.v[k];
						while(ofs<(signed int)vcount.size())
						{
				...
				loopj(0,(signed int)vcount.size(),1) if(vcount[j]==1)
	...
	void compact_mesh()
	{
		...
		if(!triangles[i].deleted)
		{
			Triangle &t=triangles[i];
			triangles[dst++]=t;
			loopj(0,3,1)vertices[t.v[j]].tcount=1;
		}
		triangles.resize(dst);
		dst=0;
		loopi(0,(signed int)vertices.size(),1)
		...
		{
			Triangle &t=triangles[i];
			loopj(0,3,1)t.v[j]=vertices[t.v[j]].tstart;
		}
		vertices.resize(dst);
	}
	...
	/*void write_obj(char* filename)
	{
		...
		loopi(0,vertices.size(),1)
		{
			fprintf(file, "v %lf %lf %lf\n", vertices[i].p.x,vertices[i].p.y,vertices[i].p.z);
		}	
		loopi(0,triangles.size(,1)) if(!triangles[i].deleted)
		{
